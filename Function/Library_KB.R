...
#----------------------------------------------------------------
train_model <- function(train, features_2, grid_final, trainmethod) {
  set.seed(10)
...
prediction_function <- function(model, data) {
  pred<- predict(model,validate[,features_2])
...
run_feature_selection <- function(train, iter, out_folder, features) {
  ...
  features_2 <- ffsResults[ffsResults$RMSE == min(ffsResults$RMSE), ]
  
  
  if (nrow(features_2) > 1) {
    features_2 <- features_2[features_2$nvar == min(features_1$nvar), ] %>%
  ...
  write.csv(features_2, sprintf('%s/featuresSelected.csv', out_loc, iter))
